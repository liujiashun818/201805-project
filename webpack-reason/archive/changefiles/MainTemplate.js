...
module.exports = class MainTemplate extends Tapable {
	constructor(outputOptions) {
		...
		this.hooks.requireExtensions.tap("MainTemplate", (source, chunk, hash) => {
			...
			if (Object.keys(chunkMaps.hash).length) {
				...
				console.log('    MainTemplate:require-ensure');	
				buf.push(
					Template.indent(
	...
	getRenderManifest(options) {
		const result = [];
		console.log('    MainTemplate:render-manifest');	
	...
	render(hash, chunk, moduleTemplate, dependencyTemplates) {
		const buf=[];
		console.log('    MainTemplate:bootstrap');	
		buf.push(
		...
		console.log('    MainTemplate:localVars');	
		buf.push(this.hooks.localVars.call("", chunk, hash));
		buf.push("");
		buf.push("// The require function");
		buf.push(`function ${this.requireFn}(moduleId) {`);
		console.log('    MainTemplate:require');	
		buf.push(Template.indent(this.hooks.require.call("", chunk, hash)));
		buf.push("}");
		buf.push("");
		console.log('    MainTemplate:require-extensions');	
		buf.push(
			Template.asString(this.hooks.requireExtensions.call("", chunk, hash))
		);
		buf.push("");
		console.log('    MainTemplate:before-startup');	
		buf.push(Template.asString(this.hooks.beforeStartup.call("",chunk,hash)));
		console.log('    MainTemplate:startup');	
		...
		if (chunk.hasEntryModule()) {
			console.log('    MainTemplate:render-with-entry');	
	...
	renderAddModule(hash,chunk,varModuleId,varModule) {
		console.log('    MainTemplate:add-module');	
	...
	renderCurrentHashCode(hash, length) {
		...
		console.log('    MainTemplate:current-hash');	
	...
	getPublicPath(options) {
		console.log('    MainTemplate:asset-path');	
	...
	updateHash(hash) {
		...
		console.log('    MainTemplate:hash');	
		this.hooks.hash.call(hash);
	}

	updateHashForChunk(hash, chunk) {
		this.updateHash(hash);
		console.log('    MainTemplate:hash-for-chunk');	
		this.hooks.hashForChunk.call(hash, chunk);
	}

	useChunkHash(chunk) {
		console.log('    MainTemplate:global-hash-paths');	
