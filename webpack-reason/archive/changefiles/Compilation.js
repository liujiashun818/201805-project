...
class Compilation extends Tapable {
	...
	buildModule(module, optional, origin, dependencies, thisCallback) {
		...
		console.log('  Compilation:build-module');
		...
		module.build(
			...
			error => {
				...
				if (error) {
					console.log('  Compilation:failed-module');
					this.hooks.failedModule.call(module, error);
					return callback(error);
				}
				console.log('  Compilation:succeed-module');
	...
	rebuildModule(module, thisCallback) {
		...
		console.log('  Compilation:rebuild-module');
		...
		this.buildModule(module, false, module, null, err => {
			if (err) {
				console.log('  Compilation:finish-rebuilding-module');
	...
	finish() {
		...
		console.log('  Compilation:finish-modules');
	...
	unseal() {
		console.log('  Compilation:unseal');
	...
	seal(callback) {
		...
		console.log('  Compilation:optimize-dependencies-basic');
		console.log('  Compilation:optimize-dependencies');
		console.log('  Compilation:optimize-dependencies-advanced');
		while (
		...
		console.log('  Compilation:after-optimize-dependencies');
		...
		console.log('  Compilation:optimize-modules-basic');
		console.log('  Compilation:optimize-modules');
		console.log('  Compilation:optimize-modules-advanced');
		while (
		...
		console.log('  Compilation:after-optimize-modules');
		this.hooks.afterOptimizeModules.call(this.modules);
		console.log('  Compilation:optimize-chunks-basic');
		console.log('  Compilation:optimize-chunks');
		console.log('  Compilation:optimize-chunks-advanced');
		while (
		...
		console.log('  Compilation:after-optimizeChunks');
		this.hooks.afterOptimizeChunks.call(this.chunks, this.chunkGroups);
		console.log('  Compilation:optimize-tree');
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			if (err) {
				return callback(err);
			}
			console.log('  Compilation:after-optimize-tree');
			this.hooks.afterOptimizeTree.call(this.chunks, this.modules);
			console.log('  Compilation:optimize-chunk-modules-basic');
			console.log('  Compilation:optimize-chunk-modules');
			console.log('  Compilation:optimize-chunk-modules-advanced');
			while (
			...
			console.log('  Compilation:after-optimize-chunk-modules');
			this.hooks.afterOptimizeChunkModules.call(this.chunks, this.modules);
			console.log('  Compilation:should-record');
			const shouldRecord = this.hooks.shouldRecord.call() !== false;
			console.log('  Compilation:revive-modules');
			this.hooks.reviveModules.call(this.modules,this.records);
			console.log('  Compilation:optimize-moduleOrder');
			this.hooks.optimizeModuleOrder.call(this.modules);
			console.log('  Compilation:advanced-optimize-module-order');
			this.hooks.advancedOptimizeModuleOrder.call(this.modules);
			console.log('  Compilation:before-moduleIds');
			this.hooks.beforeModuleIds.call(this.modules);
			console.log('  Compilation:module-ids');
			this.hooks.moduleIds.call(this.modules);
			this.applyModuleIds();
			console.log('  Compilation:optimize-module-ids');
			this.hooks.optimizeModuleIds.call(this.modules);
			console.log('  Compilation:after-optimize-module-ids');
			this.hooks.afterOptimizeModuleIds.call(this.modules);

			this.sortItemsWithModuleIds();
			console.log('  Compilation:revive-chunks');
			this.hooks.reviveChunks.call(this.chunks,this.records);
			console.log('  Compilation:optimize-chunk-order');
			this.hooks.optimizeChunkOrder.call(this.chunks);
			console.log('  Compilation:before-chunk-ids');
			this.hooks.beforeChunkIds.call(this.chunks);
			this.applyChunkIds();
			console.log('  Compilation:optimize-chunk-ids');
			this.hooks.optimizeChunkIds.call(this.chunks);
			console.log('  Compilation:after-optimize-chunk-ids');
			this.hooks.afterOptimizeChunkIds.call(this.chunks);

			this.sortItemsWithChunkIds();
			console.log('  Compilation:record-modules');
			if (shouldRecord)
				this.hooks.recordModules.call(this.modules,this.records);
			console.log('  Compilation:record-chunks');
			if (shouldRecord) this.hooks.recordChunks.call(this.chunks, this.records);
			console.log('  Compilation:before-hash');
			this.hooks.beforeHash.call();
			this.createHash();
			console.log('  Compilation:after-hash');
			this.hooks.afterHash.call();
			console.log('  Compilation:record-hash');
			if (shouldRecord) this.hooks.recordHash.call(this.records);
			console.log('  Compilation:before-module-assets');
			this.hooks.beforeModuleAssets.call();
			this.createModuleAssets();
			console.log('  Compilation:should-generate-chunk-assets');
			if (this.hooks.shouldGenerateChunkAssets.call()!==false) {
				console.log('  Compilation:before-chunk-assets');
				this.hooks.beforeChunkAssets.call();
				this.createChunkAssets();
			}
			console.log('  Compilation:additional-chunk-assets');
			this.hooks.additionalChunkAssets.call(this.chunks);
			this.summarizeDependencies();
			console.log('  Compilation:record');
			if (shouldRecord) this.hooks.record.call(this, this.records);
			console.log('  Compilation:additional-assets');
			this.hooks.additionalAssets.callAsync(err => {
				if (err) {
					return callback(err);
				}
				console.log('  Compilation:optimize-chunk-assets');
				this.hooks.optimizeChunkAssets.callAsync(this.chunks, err => {
					if (err) {
						return callback(err);
					}
					console.log('  Compilation:after-optimize-chunk-assets');
					this.hooks.afterOptimizeChunkAssets.call(this.chunks);
					console.log('  Compilation:optimize-assets');
					this.hooks.optimizeAssets.callAsync(this.assets, err => {
						if (err) {
							return callback(err);
						}
						console.log('  Compilation:after-optimize-assets');
						this.hooks.afterOptimizeAssets.call(this.assets);
						console.log('  Compilation:need-additional-seal');
						if (this.hooks.needAdditionalSeal.call()) {
							this.unseal();
							return this.seal(callback);
						}
						console.log('  Compilation:after-seal');
	...
	createHash() {
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			console.log('  Compilation:chunk-hash');
		...
		for (let i = 0; i < this.modules.length; i++) {
			...
			if (module.buildInfo.assets) {
				for (const assetName of Object.keys(module.buildInfo.assets)) {
					...
					console.log('  Compilation:module-asset');
		...
		for (let i = 0; i < this.chunks.length; i++) {
			...
			try {
				...
				for (const fileManifest of manifest) {
					...
					console.log('  Compilation:chunk-asset');
